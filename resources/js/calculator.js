...
const addItemToList = (data, type, list) => {
    let price;
...
const visablePrice = (array) => {
    ...
    const specialTypes = ['wall', 'сeiling', 'floor'];
    $.map(array, (data, type) => {
        if (data.length == 0) return false;

        if(specialTypes.indexOf(type) !== -1 && data instanceof Array) {
            data.forEach((dataItem) => addItemToList(dataItem, type, list))
        } else {
            addItemToList(data, type, list);
        }
    });
